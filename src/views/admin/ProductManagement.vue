...
<main class="container mt-3">
  ...
  data-bs-target="#addProductModal">新增產品</button>
  ...
  <label class="form-check-label" for="flexSwitchCheckChecked">
    {{ item.is_enabled ? '上架' : '未上架'}}
        ...
        <th>
          <button class="btn btn-outline-success me-1" @click="getEditProduct(item.id)"
          data-bs-toggle="modal" data-bs-target="#productEditModal"
          >編輯</button>
          <button class="btn btn-primary" @click="deleteProduct(item)">刪除</button>
        </th>
      </tr>
      </tbody>
  </table>
...
<!-- productEditModal v-if="tempProduct.imageUrl" -->
  <div id="productEditModal" ref="productEditModal"
          ...
          <div class="row">
              ...
              <div v-if="Array.isArray(tempProduct.imagesUrl)">
                ...
                <div class="mb-1" v-for="(item, i) in tempProduct.imagesUrl" :key="i">
                  <div class="form-group">
                    ...
                    <input type="text" class="form-control" v-model="tempProduct.imagesUrl[i]"
                    placeholder="請輸入圖片連結">
                  </div>
                  <!-- :src="item" -->
                ...
                <div >
                  ...
                  d-block w-100" @click="tempProduct.imagesUrl.pop()">
                    刪除圖片
                  </button>
                </div>
              </div>
            ...
            <div class="col-sm-8">
              <div class="form-group">
                ...
                <input id="title"  type="text" v-model="tempProduct.title"
              ...
              <div class="row">
                <div class="form-group col-md-6">
                  ...
                  type="number" min="0" v-model.number="tempProduct.origin_price"
                    class="form-control" placeholder="請輸入原價">
                </div>
                <div class="form-group col-md-6">
                  <label for="price">售價</label>
                  <!-- v-model.number="tempProduct.price" -->
                  <input id="price" v-model.number="tempProduct.price"
          ...
          <div class="row">
            <div class="col-sm-4">
              ...
              <div v-if="Array.isArray(addProductModal.data.imagesUrl)">
                ...
                <!-- 使用 v-for 跑 tempProduct.imagesUrl 來對應按下新增圖片按鈕產生空欄位-->
                <!-- v-for="(item, i) in tempProduct.imagesUrl" :key="i" -->
                <div class="mb-1" v-for="(item, i) in addProductModal.data.imagesUrl" :key="i">
                  <div class="form-group">
                    <label for="imageUrl">圖片網址</label>
                    <!-- v-model="tempProduct.imagesUrl[i]" -->
                    <input type="text" class="form-control"
                ...
                <!-- v-if="!tempProduct.imagesUrl.length ||
                tempProduct.imagesUrl[tempProduct.imagesUrl.length - 1]" -->
                <div v-if="!addProductModal.data.imagesUrl.length ||
                addProductModal.data.imagesUrl[addProductModal.data.imagesUrl.length - 1]">
                  <!-- 使用 push加入imagesUrl陣列末端-->
                  <!-- @click="tempProduct.imagesUrl.push('')" -->
                  <button class="btn btn-outline-primary btn-sm d-block w-100"
                ...
                <div >
                  ...
                  <!-- @click="tempProduct.imagesUrl.pop()" -->
                  <button class="btn btn-outline-danger btn-sm
                  d-block w-100" @click="addProductModal.data.imagesUrl.pop()">
                    刪除圖片
                  </button>
                </div>
              </div>
            ...
            <div class="col-sm-8">
              <div class="form-group">
                <label for="title">標題</label>
                <!-- v-model="tempProduct.title" -->
                <input id="addProductModalTitle"  type="text" v-model="addProductModal.data.title"
                class="form-control" placeholder="請輸入標題">
              </div>
              <div class="row">
                <div class="form-group col-md-6">
                  <label for="category">分類</label>
                  <!-- v-model="tempProduct.category" -->
                  <input id="addProductModalCategory" v-model="addProductModal.data.category"
                  type="text" class="form-control"
                    placeholder="請輸入分類">
                </div>
                <div class="form-group col-md-6">
                  <label for="price">單位</label>
                  <!-- v-model="tempProduct.unit" -->
                  <input id="addProductModalUnit" v-model="addProductModal.data.unit"
                  type="text" class="form-control" placeholder="請輸入單位">
                </div>
              </div>
              <div class="row">
                <div class="form-group col-md-6">
                  <label for="origin_price">原價</label>
                  <!-- v-model.number="tempProduct.origin_price" -->
                  <input id="addProductModalOrigin_price"
                  type="number" min="0" v-model.number="addProductModal.data.origin_price"
                    class="form-control" placeholder="請輸入原價">
                </div>
                <div class="form-group col-md-6">
                  <label for="price">售價</label>
                  <!-- v-model.number="tempProduct.price" -->
                  <input id="addProductModalPrice" v-model.number="addProductModal.data.price"
                  type="number" min="0" class="form-control"
                    placeholder="請輸入售價">
                </div>
              </div>
              ...
              <div class="form-group">
                <label for="description">產品描述</label>
                <!-- v-model="tempProduct.description" -->
                <textarea id="addProductModalDescription" v-model="addProductModal.data.description"
                type="text" class="form-control"
                  placeholder="請輸入產品描述">
                    </textarea>
              </div>
              <div class="form-group">
                <label for="content">說明內容</label>
                <!-- v-model="tempProduct.content" -->
                <textarea id="addProductModalDescription" v-model="addProductModal.data.content"
                type="text" class="form-control"
                  placeholder="請輸入說明內容">
                    </textarea>
              </div>
              <div class="form-group">
                <div class="form-check">
                  <!-- v-model="tempProduct.is_enabled" :true-value="1" :false-value="0"-->
                  <input id="is_enabled" v-model="addProductModal.data.is_enabled"
                  class="form-check-input" type="checkbox" :true-value="1" :false-value="0"
                    >
                  <label class="form-check-label" for="is_enabled">是否啟用</label>
                </div>
